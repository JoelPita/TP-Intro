...
def pedir_precios_habitaciones():
    obtener_habitaciones_api = api_URL('gestion_precios/obtener-habitaciones')
    try:                       
        response = requests.get(obtener_habitaciones_api)
...
def modificar_precios_habitaciones():
    ...
    actualizar_precio_api = api_URL('gestion_precios/gestion_precios')
                 
    try:
        response = requests.patch(actualizar_precio_api, json={'id': id_habitacion, 'precio_noche': nuevo_precio})
